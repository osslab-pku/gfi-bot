...
def get_training_result(name:Union[None, str]=None, owner: Union[None, str]=None):
    ...
    if name != None and owner != None:
        query: TrainingSummary = TrainingSummary.objects(Q(name=name, owner=owner)).only(*TrainingResult.__fields__).order_by("-threshold").first()
        if not query:
        ...
        for repo in Repo.objects():
            query: TrainingSummary = TrainingSummary.objects(Q(name=repo.name, owner=repo.owner)).only(*TrainingResult.__fields__).order_by("-threshold").first()
            if query:
                q = {**query.to_mongo()}
                q["issues_train"] = len(q["issues_train"])
                q["issues_test"] = len(q["issues_test"])
