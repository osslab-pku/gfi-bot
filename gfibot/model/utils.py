...
def getdataset(
    ...
    res = {
        ...
        "clst": clst,
        "pro_star": pro_star,
        "proissnewnum": proissnewnum,
        "propr": propr,
        "rptcmt": rptcmt,
        "rptiss": rptiss,
        "rptpr": rptpr,
        "rptnpratio": rptnpratio,
        "rptisnew": rptisnew,
        "proissnewratio": proissnewratio,
        "ownercmt": ownercmt,
        "owneriss": owneriss,
        "ownerpr": ownerpr,
        "ownerissnewnum": ownerissnewnum,
        "ownerissnewratio": ownerissnewratio,
        "procmt": procmt,
...
def load_df(
    newthr,
):  # extract data frome gfibot.issuedataset for each issue to further build the training data set and test data set with def(load_train_test_data)
    ...
    issuelist = [
        [
            ...
            x["NumOfUrls"],
            x["NumOfPics"],
            x["coleman_liau_index"],
            x["flesch_reading_ease"],
            x["flesch_kincaid_grade"],
            x["automated_readability_index"],
            x["LengthOfTitle"],
            x["LengthOfDescription"],
            x["commentnum"],
            x["title"],
            x["body"],
            x["comment"],
    ...
    rowdf1 = rowdf[
        ["ifnew", "owner", "name", "number", "title", "body", "comment", "clst"]
    ]
    rowdf2 = rowdf[
        [
            "pro_star",
    ...
    c = []  # add column names for dataframe
...
def load_train_test_data(df, settype, owner, name):
    if settype == "one4one":
        ...
        ind = df[
            (df["owner"] == owner) & (df["name"] == name) & (df["number"] == number)
        ].index.tolist()[0]
    ...
    print(
        train_data[train_data.ifnew == 1].shape[0],
        train_data[train_data.ifnew == 0].shape[0],
        test_data.shape[0],
    )  # ifnew=1和0的数量
